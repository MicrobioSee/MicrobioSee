...
pcaplot<-function(file_abun,file_group,scale,ellipse,addlabel,labelposition,labelsize){
  ...
  pcagroup<-read.table(file_group,header = T,sep = "\t")
  pca_pcoaOUTult <- prcomp(df, scale=T ) # 将变量缩放到具有单位方差
  g=ggbiplot(pca_pcoaOUTult,
             var.axes = F,
             obs.scale = scale,         # 横纵比例
             groups = pcagroup[,2],  # 添加分组信息，为分组文件的第一列
...
strss_value<- function(ndata,ndata.group,standardization,dist){
  spe <- read.table(ndata,header = TRUE,sep = "\t",row.names = 1)#读取
  ...
  return(paste0("The StpcoaOUTs value is ",spe.h.nmds$stpcoaOUTs))
}
nmds <- function(ndata,ndata.group,standardization,dist){
  ...
  #NMDS分析
  spe.h.nmds <- metaMDS(spe.h,distance=dist)#NMDS分析-基于Bray-Curtis距离
  map<- scopcoaOUT(spe.h.nmds)#提取前两轴坐标/spe.h.nmds$points#查看前两轴物种
  #ggplot2绘图
  ...
  ggplot(data = map, aes(NMDS1, NMDS2))+
    ...
    # geom_text(aes(x=3,y=2,label=paste0("stpcoaOUTs = ",spe.h.nmds$stpcoaOUTs)))+ #设置文字位置、内容和大小
...
nmds.stpcoaOUTs <- function(ndata,ndata.group,standardization,dist){
  spe <- read.table(ndata,header = TRUE,sep = "\t",row.names = 1)#读取物种数据
  ...
  #NMDS分析
  spe.h.nmds <- metaMDS(spe.h,distance=dist)#NMDS分析-基于Bray-Curtis距离
  # spe.h.nmds$stpcoaOUTs#查看stpcoaOUTs值
  stpcoaOUTsplot(spe.h.nmds, main = "Shepard")
}
